# 掘金发布 — 直接用 Markdown 粘贴

## 操作步骤:
1. 登录 https://juejin.cn
2. 点击右上角 "写文章"
3. 粘贴以下内容
4. 标题: 我用 AI Agent 搭建了 A 股实时看盘系统
5. 标签: Go, Electron, React, AI, 量化交易
6. 点击 "发布"
...
前端用 TanStack Query + WebSocket hook，实现了 **断线自动重连** 和 **增量更新**。
...
AI 对话集成的是 oMLX 本地 LLM（Claude API 格式），支持流式输出。
...
- **PyInstaller** 打包成单文件可执行 (314MB, akshare + torch)
...
本地 LLM 的思维链输出格式不稳定——有时有 `Here's a thinking process`，有时没有。写了 `stripThinking()` 函数做容错解析。
...
📥 [v1.0 下载](https://github.com/chenqiangxu219-design/a-share-assistant/releases/tag/v1.0.0)

**如果你也在用 AI Agent 做开发，欢迎 Star + 交流。** 🚀

---

*本文首发于掘金。项目采用 MPL-2.0 协议开源。*
